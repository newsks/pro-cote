...
function solution(priorities, location) {
  ...
  while (queue.length > 0) {
    ...
    if (queue.some((process) => process.priority > current.priority)) {
      // 우선순위가 높은 프로세스가 있다면, 현재 프로세스를 큐의 끝에 다시 넣음
      queue.push(current);
    } else {
      // 꺼낸 프로세스를 실행 (실제로는 순서만 기록)
      order++;
      // 현재 실행한 프로세스가 찾고자 하는 위치의 프로세스인지 확인
      if (current.index === location) {
        return order; // 찾고자 하는 프로세스의 실행 순서 반환
      }
    }
  }
}
